...
<head>
    ...
    <style>
        ...
        .header {
            background-color: #0F2C59; /* Warna M2B */
            color: #ffffff;
            padding: 20px;
            text-align: center;
        }
        ...
        .details-box {
            ...
            border-left: 4px solid #B91C1C; /* Aksen Merah */
            padding: 15px;
            margin: 20px 0;
            font-size: 14px;
        }
...
<body>
    <div class="email-wrapper">
        <div class="container">
            ...
            <div class="content">
                <p>Halo Pak/Bu <strong>{{ $quotation->customer ? $quotation->customer->user->name : $quotation->manual_pic }}</strong>,</p>
                
                <p>Semoga hari Anda menyenangkan.</p>

                <p>Menindaklanjuti diskusi kita mengenai kebutuhan <strong>{{ ucfirst($quotation->service_type) }}</strong>, bersama email ini kami lampirkan penawaran harga (Quotation) terbaik dari kami.</p>

                <div class="details-box">
                    <strong>Ringkasan Penawaran:</strong><br>
                    Nomor: {{ $quotation->quotation_number }}<br>
                    Rute: {{ $quotation->origin }} &rarr; {{ $quotation->destination }}<br>
                    Berlaku Hingga: {{ $quotation->valid_until->format('d M Y') }}
                </div>
