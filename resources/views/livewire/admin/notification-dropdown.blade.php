    ...
    <div x-show="open" @click.away="open = false" 
        ...
        <div class="max-h-80 overflow-y-auto">
            ...
            @else
                ...
                {{-- 3. QUOTATION DISETUJUI --}}
                @foreach($approvedQuotations as $qt)
                <a href="{{ route('admin.quotations.index') }}" class="block p-3 hover:bg-emerald-50 border-b border-gray-50 transition group">
                    <div class="flex gap-3">
                        <div class="bg-emerald-100 text-emerald-600 p-2 rounded-full h-8 w-8 flex items-center justify-center shrink-0">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-800 group-hover:text-emerald-700">✅ Quotation Disetujui!</p>
                            <p class="text-[10px] text-gray-500">{{ $qt->quotation_number }} • {{ $qt->manual_company ?? $qt->customer?->company_name ?? '-' }}</p>
                            <p class="text-[9px] text-emerald-500 mt-1 font-semibold">{{ $qt->approved_at?->diffForHumans() }} · Rp {{ number_format($qt->grand_total) }}</p>
                        </div>
                    </div>
                </a>
                @endforeach
