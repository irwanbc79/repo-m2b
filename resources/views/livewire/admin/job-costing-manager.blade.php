    ...
    @if($isModalOpen && $selectedShipment)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm overflow-y-auto" wire:ignore.self>
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl animate-fade-in-up my-8">
                ...
                <div class="p-6 grid grid-cols-1 {{ auth()->user()->hasRole('auditor') ? '' : 'lg:grid-cols-3' }} gap-6 max-h-[80vh] overflow-y-auto">
                    
                    {{-- KOLOM KIRI: FORM INPUT (disembunyikan untuk auditor) --}}
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-center align-top">
                                            <div class="flex items-center justify-center gap-1">
                                                ...
                                                {{-- Rate Vendor Button --}}
                                                @if($cost->vendor_id)
                                                    <button wire:click="openRatingModal({{ $cost->vendor_id }}, {{ $cost->id }})" 
                                                        class="text-yellow-500 hover:text-yellow-600 transition p-1 rounded hover:bg-yellow-50" 
                                                        title="Rate Vendor: {{ $cost->vendor->name ?? '' }}">
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-yellow-400 to-orange-500 px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-white font-bold text-lg">Rate Vendor</h3>
                                <p class="text-yellow-100 text-sm">{{ $ratingVendorName }}</p>
                            </div>
                        </div>
                        <button wire:click="closeRatingModal" class="text-white hover:text-yellow-200 text-2xl">&times;</button>
                    </div>
                </div>
                
                {{-- Body --}}
                <div class="p-6">
                    <p class="text-gray-600 text-sm mb-4">Bagaimana kinerja vendor ini untuk job ini?</p>
                    
                    {{-- Star Rating --}}
                    <div class="flex justify-center gap-2 mb-6">
                        @for($i = 1; $i <= 5; $i++)
                            <button 
                                wire:click="$set('vendorRating', {{ $i }})"
                                class="text-4xl transition-transform hover:scale-110 focus:outline-none {{ $vendorRating >= $i ? 'text-yellow-400' : 'text-gray-300' }}">
                                ★
                            </button>
                        @endfor
                    </div>
                    ...
                    <div class="text-center mb-4">
                        <span class="inline-block px-4 py-2 rounded-full text-sm font-medium
                            @if($vendorRating == 5) bg-green-100 text-green-700
                            @elseif($vendorRating == 4) bg-blue-100 text-blue-700
                            @elseif($vendorRating == 3) bg-yellow-100 text-yellow-700
                            @elseif($vendorRating == 2) bg-orange-100 text-orange-700
                            @else bg-red-100 text-red-700 @endif">
                            @if($vendorRating == 5) Sangat Baik! 🎉
                            @elseif($vendorRating == 4) Baik 👍
                            @elseif($vendorRating == 3) Cukup 😐
                            @elseif($vendorRating == 2) Kurang 😕
                            @else Buruk 😞 @endif
                        </span>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea 
                            wire:model="ratingNotes" 
                            rows="3" 
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400"
                            placeholder="Contoh: Pengiriman tepat waktu, komunikasi baik..."></textarea>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 px-6 py-4 flex justify-between">
                    ...
                    <button 
                        ...
                        class="px-6 py-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-white rounded-lg font-bold text-sm hover:from-yellow-500 hover:to-orange-600 transition shadow-md">
                        💾 Simpan Rating
                    </button>
                </div>
            </div>
