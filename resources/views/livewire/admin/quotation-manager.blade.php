<div class="space-y-6">
    ...
    @php
        ...
        $activeQuotationDateLabel = $presetNames[$datePreset] ?? (($filterDateFrom || $filterDateTo) ? (($filterDateFrom ?: '...') . ' s/d ' . ($filterDateTo ?: '...')) : null);
    @endphp

    @if($activeQuotationDateLabel)
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 text-xs text-blue-900 shadow-sm mb-4">
        <div class="flex items-center gap-2">
            ...
            <span class="leading-relaxed">Menampilkan data & statistik untuk periode: <strong>{{ $activeQuotationDateLabel }}</strong> @if($filterDateFrom && $filterDateTo)({{ \Carbon\Carbon::parse($filterDateFrom)->translatedFormat('d M Y') }} &minus; {{ \Carbon\Carbon::parse($filterDateTo)->translatedFormat('d M Y') }})@endif</span>
        </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-1.5 bg-white border border-gray-300 rounded-lg px-2.5 py-1 shadow-sm">
                    ...
                    <div class="flex items-center gap-1 pl-2 border-l border-gray-200">
                        <input type="date" wire:model.live="filterDateFrom" class="border border-gray-200 rounded px-2 py-0.5 text-xs text-gray-700 focus:ring-1 focus:ring-blue-500" title="Dari Tanggal">
                        <span class="text-gray-400 text-xs">-</span>
                        <input type="date" wire:model.live="filterDateTo" class="border border-gray-200 rounded px-2 py-0.5 text-xs text-gray-700 focus:ring-1 focus:ring-blue-500" title="Sampai Tanggal">
                    </div>
                    @endif
                ...
                @if($search || $filterStatus || $filterType || $datePreset || $filterDateFrom || $filterDateTo)
                            ...
                            <div class="flex justify-center gap-2 items-center">
                                ...
                                <div wire:ignore x-data="{ open: false }" class="relative inline-block">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-cloak class="absolute right-0 mt-1 w-56 bg-white border rounded-xl shadow-xl z-50 py-2">
                                        <button @click="open = false; $dispatch('open-print-preview', { url: '{{ route('admin.quotations.print', $q->id) }}?signature=full&signer=1' })" class="w-full text-left block px-4 py-2 hover:bg-green-50 text-xs text-gray-700 transition">
                                            <span class="font-bold text-green-700">✓ Sign + Stamp</span><br>
                                            <span class="text-[10px] text-gray-400">Nurul Asyikin</span>
                                        </button>
                                        <button @click="open = false; $dispatch('open-print-preview', { url: '{{ route('admin.quotations.print', $q->id) }}?signature=blank&signer=1' })" class="w-full text-left block px-4 py-2 hover:bg-gray-50 text-xs text-gray-700 transition">
    ...
    <div x-data="{ show: false, url: '' }" 
        ...
        <div class="bg-white w-full max-w-4xl rounded-xl shadow-2xl flex flex-col" style="height: 90vh;">
            {{-- Header --}}
            <div class="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
                <h3 class="font-bold text-lg text-m2b-primary flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    Preview Quotation
                </h3>
                <div class="flex items-center gap-2">
                    <button @click="$refs.printFrame.contentWindow.print()" class="bg-m2b-primary hover:bg-blue-900 text-white px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 shadow-sm transition">
